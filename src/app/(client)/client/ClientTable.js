...
function normalizeMonth(str) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, filterStatus, sortLocation, clients]);

  const filteredClients = clients
    .filter((c) =>
      (c.name || "").toLowerCase().includes(searchQuery.trim().toLowerCase()) ||
      (c.contact || "").toLowerCase().includes(searchQuery.trim().toLowerCase()) ||
       normalizeMonth(c.date || "").includes(normalizeMonth(searchQuery.trim().toLowerCase())) ||
      (c.machine || "").toLowerCase().includes(searchQuery.trim().toLowerCase()) ||
      (c.client || "").toLowerCase().includes(searchQuery.trim().toLowerCase()) ||
      (c.location || "").toLowerCase().includes(searchQuery.trim().toLowerCase())
    )
    .filter((c) => filterStatus === "All" ? true : c.report === filterStatus)
    .filter((c) =>
      sortLocation === "All"
        ? true
        : (c.location || "").toLowerCase() === sortLocation.toLowerCase()
    );
    ...
    <div className="py-4 w-full">
      ...
      <div className="flex flex-col md:flex-col lg:flex-row md:px-20 justify-between items-center mt-4">
        <div className="text-base pb-5 md:text-sm text-[#b5b7c0]">
          Showing {indexOfFirstRow + 1} to {Math.min(indexOfLastRow, filteredClients.length)} of {filteredClients.length} entries
        </div>
